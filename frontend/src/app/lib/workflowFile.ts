...
import type { WorkflowFile } from "./mikeApi";

const WORKFLOW_FILE_FORMAT = "mikelocal.workflow" as const;
...
export function buildWorkflowEnvelope(wf: MikeWorkflow): WorkflowFile {
    return {
        ...
        source_label: wf.source_label ?? null,
...
export function parseWorkflowFile(raw: string): WorkflowFile {
    ...
    if (
        obj.columns_config != null &&
        !Array.isArray(obj.columns_config)
    ) {
    ...
    return {
        ...
        source_label: (obj.source_label as string | null | undefined) ?? null,
        prompt_md: (obj.prompt_md as string | null | undefined) ?? null,
        columns_config:
            (obj.columns_config as WorkflowFile["columns_config"]) ?? null,
    };
}
